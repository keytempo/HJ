...
// How far past the last panel's own bottom edge (i.e. once there's no next
// panel to blend into) the tail glow's fade to transparent completes. This
// is a fixed distance rather than a proportion of the tail's total height
// deliberately — see the mask-computation comment in buildAmbientGlow for
// why a proportional fade breaks badly on a tall last panel.
const GLOW_TAIL_FADE = 420;
...
// setViewerState is exclusively the error-reporting path — see
// dismissViewerState below for the (silent, textless) happy path. Errors
// are the one case in the initial-load flow where the person genuinely
// needs to read something and possibly act on it (retry), so the full card
// treatment (see styles.css .is-error) is reserved for this.
...
function createPanelImage(panel, panelIndex) {
  ...
  image.addEventListener(
    "error",
    () => {
      // If this panel hadn't been fully handled yet (e.g. the far tier
      // started the fetch but the near tier hadn't reached it), drop it
      // from both observers — unobserve() on a target that isn't currently
      // observed is a harmless no-op, so this is safe regardless of which
      // tier(s) already fired. Otherwise an observer could sit holding a
      // reference to this element indefinitely once it's replaced below.
      farObserver?.unobserve(image);
      nearObserver?.unobserve(image);
...
// constant. Margins apply above and below alike: reading is normally
// forward-only, so the upward margin rarely has anything left to do (by
// the time a panel is behind the reader, forward preloading already
// reached it on the way past) — but it matters whenever the viewport can
// land somewhere without passing through everything above it first, e.g.
// a browser restoring scroll position deep in an episode after a reload,
// or a jump-to-top/bottom key. Without it, panels above that landing point
// would have no lookahead at all in that direction. Fast scrolling doesn't
// need separate handling either way: intersection observers fire as soon
// as an element crosses the (generous) threshold regardless of how quickly
// it got there, with no scroll listener or per-frame polling involved.
// ---------------------------------------------------------------------
...
function setupPreloading(images) {
  ...
  farObserver = new IntersectionObserver(
    ...
    { rootMargin: `${far}px 0px ${far}px 0px` },
  );
  ...
  nearObserver = new IntersectionObserver(
    ...
    { rootMargin: `${near}px 0px ${near}px 0px` },
  );
...
//
// The final panel's glow is a special case: it's stretched down to (at
// least) episode-end's guaranteed minimum height, so the atmosphere
// already carries roughly through the footer from the very first paint —
// see the reservedFooterHeight comment below for why this is calculated
// ahead of episode-end actually being visible, rather than only reacting
// to it afterward. Calling this again (or extendGlowThroughFooter, its
// cheaper companion) once episode-end has its real, final height is what
// closes the small remaining gap if its actual content runs taller than
// that reserved floor.
function buildAmbientGlow() {
  ...
  // While episode-end is still hidden (true for this function's very first
  // call on every episode load — manifest, which is what episode-end needs,
  // is only awaited well after this point), documentHeight reflects the
  // strip alone. Reserving episode-end's own guaranteed CSS min-height
  // ahead of time — read live, not hard-coded, so it can't drift out of
  // sync with the stylesheet — means the tail glow already extends roughly
  // as far as it's going to from the very first paint, instead of jumping
  // visibly later if the person reaches the bottom before episode-end's
  // real content (which does typically run a little taller than this
  // floor) is known. Once episode-end is actually visible, its real height
  // is already part of documentHeight and nothing extra is added here.
  const reservedFooterHeight = episodeEnd.hidden
    ? Number.parseFloat(getComputedStyle(episodeEnd).minHeight) || 0
    : 0;
  const projectedDocumentHeight = documentHeight + reservedFooterHeight;
  const layers = document.createDocumentFragment();
  // Tracks the last glow's actual bottom edge (top + height), which the
  // Math.max below can push past documentHeight — ambient's own height is
  // set to this afterward rather than assumed to equal documentHeight, so
  // that case can never clip back off what Math.max just extended.
  let ambientHeight = documentHeight;

  images.forEach((image, index) => {
    const bounds = image.getBoundingClientRect();
    const top = bounds.top + scrollY - GLOW_OVERLAP;
    // The last glow's bottom edge is made to land at the (possibly
    // footer-reserving) projected document height — rather than just
    // +GLOW_OVERLAP past its own panel — so it lines up with ambient's own
    // height below with nothing left to clip; its mask (see
    // .ambient__glow--tail in styles.css) finishes fading to transparent a
    // little before that edge regardless.
    //
    // Math.max guards the case where even the reservation above still
    // falls short of the plain (non-extending) formula below for this
    // panel — e.g. a very tall last panel on a viewport where glow is only
    // just barely supported. Taking whichever is larger means this never
    // regresses below that baseline.
    const isLast = index === images.length - 1;
    const height = isLast
      ? Math.max(projectedDocumentHeight - top, bounds.height + GLOW_OVERLAP * 2)
      : bounds.height + GLOW_OVERLAP * 2;
    if (isLast) ambientHeight = top + height;
    ...
    if (isLast) {
      // Every other panel stays at a flat, stable opacity for the entire
      // time it's being read — only the overlap with the *next* panel's
      // glow ever covers part of it, which isn't a fade, just one layer
      // drawn over another. The last panel has no next panel to be
      // (harmlessly) covered by, so it fades instead — but that fade must
      // be anchored to a fixed distance past its OWN bottom edge
      // (bounds.height, in this element's local coordinates), not to a
      // percentage of the tail's total height. A percentage-based mask
      // looked fine for ordinary panels, but on a tall last panel it starts
      // fading while the reader is still well inside that very panel,
      // long before reaching the footer — a visible, panel-by-panel
      // inconsistency a percentage can't avoid but a fixed pixel distance
      // does, by construction, regardless of how tall the panel or how far
      // the tail extends beyond it.
      const fadeStart = bounds.height + GLOW_OVERLAP * 2;
      const fadeEnd = fadeStart + GLOW_TAIL_FADE;
  ...
  ambient.style.height = `${ambientHeight}px`;
}

// A cheap companion to buildAmbientGlow(), used only for the one moment the
// episode-end section goes from hidden to its real, final height (see
// initializeViewer). Rebuilding every panel's glow again for that would
// re-measure and re-create N elements just to change one of them — this
// instead only touches the tail glow already in place (extending it to the
// document's new true bottom) and the ambient container's height to match.
// Safe to call even if nothing has actually grown: it's a no-op in that
// case, since Math.max below never lets the tail shrink relative to
// whatever it was already showing.
function extendGlowThroughFooter() {
  const tail = ambient.querySelector(".ambient__glow--tail");
  if (!tail) return; // no glow built yet (unsupported viewport, or no panels)

  const top = Number.parseFloat(tail.style.top);
  const currentHeight = Number.parseFloat(tail.style.height);
  const documentHeight = document.documentElement.scrollHeight;
  // Math.max, not a direct assignment: documentHeight - top is only ever
  // meant to grow the tail (episode-end going from hidden to visible always
  // adds height, never removes it), but deriving that guarantee from
  // episode-end's own CSS minimums would make this silently wrong the day
  // those minimums ever change. Taking whichever is larger holds regardless
  // of what produced the current height, and moving the ambient container's
  // own height to match the tail's new true bottom edge (rather than
  // documentHeight directly) is what keeps ambient's overflow:hidden from
  // ever clipping back off whatever Math.max just extended.
  const height = Math.max(documentHeight - top, currentHeight);
  tail.style.height = `${height}px`;
  ambient.style.height = `${top + height}px`;
}
...
async function initializeViewer() {
  ...
  try {
    ...
    // height — this is what lets the last panel's glow stretch all the way
    // to the page's true bottom instead of wherever the document ended
    // while episode-end was still hidden (see extendGlowThroughFooter).
    extendGlowThroughFooter();
  } catch (error) {
    console.error(error);
